(function(){
    com.webrtc.WIMSSession = Object.subClass({
        ...
        Call : function(CalleeId,Config,CallerCalleeType)
        {
            this.Confs=Config;
            console.log(this.Confs);
            this.callername = CalleeId;
            //收到offer，发送名为answer的offer
            if(CallerCalleeType == "Called"){
                if(this.Confs.moduletype == com.webrtc.UserSession.MODULE_TYPE.IMSVIDEO)
                {
                    this.moduleList[this.SessionID] = new com.webrtc.IMSVideoModule(this.SessionID,CalleeId,this.Confs,CallerCalleeType);
                ...
                {
                    this.moduleList[this.SessionID] = new com.webrtc.IMSAudioModule(this.SessionID,CalleeId,this.Confs,CallerCalleeType);
                    this.moduleList[this.SessionID].SendMessageToUserSession = this.sendMessage;
                    this.moduleList[this.SessionID].captureLocalMedia(); 
                }
                else
        ...
        handleMessage:function(message)
        {
            ...
            if(msg.roap.type === com.webrtc.protocol.RTCRoapType['offer']){
                ...
                if(moduleType == com.webrtc.UserSession.MODULE_TYPE.IMSVIDEO)
                {
                    ...
                    {
                        //被叫Conf参数：sessiontype,moduletype,caller,booldtls,iceservers,booldatechannel
